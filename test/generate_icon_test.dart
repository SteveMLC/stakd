...
void main() {
  test('Generate app icon', () async {
    ...
    final bgPaint = Paint()
      ..shader = ui.Gradient.radial(Offset(size / 2, size / 2), size * 0.6, [
    ...
    for (int i = 0; i < 5; i++) {
      ...
      final layerPaint = Paint()
        ..shader = ui.Gradient.linear(
          ...
          Offset(centerX + layerWidth / 2, y),
          [colors[i], _adjustBrightness(colors[i], 1.2)],
        )
        ..style = PaintingStyle.fill;

      // Draw rounded rectangle for layer
      final rect = RRect.fromRectAndRadius(
        Rect.fromLTWH(centerX - layerWidth / 2, y, layerWidth, layerHeight),
    ...
    canvas.drawCircle(Offset(centerX, centerY), size * 0.40, zenCirclePaint);
...
Color _adjustBrightness(Color color, double factor) {
  int scale(double channel) => (channel * 255.0 * factor).round().clamp(0, 255);
