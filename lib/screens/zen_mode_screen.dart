...
class _ZenModeScreenState extends State<ZenModeScreen>
  ...
  List<GameStack> _generateZenPuzzle() {
    // Use custom seed for variety
    final generator = LevelGenerator(seed: _puzzleSeed);
    
    // Create a pseudo-level based on difficulty
    // Easy = level 5, Medium = level 25, Hard = level 60
    final pseudoLevel = switch (_difficulty) {
      ZenDifficulty.easy => 5 + _random.nextInt(5),
      ZenDifficulty.medium => 20 + _random.nextInt(10),
      ZenDifficulty.hard => 55 + _random.nextInt(20),
    };
    
    return generator.generateSolvableLevel(pseudoLevel);
  }
